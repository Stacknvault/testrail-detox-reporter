...
class Utils {
  ...
  _formatTitle(title) {
    const regex = /^C([^ ]*)/;
    const case_id = [title.match(regex)[1]];
    return case_id;
  }
    ...
    for (let i = 0, len = jest_result_list.length; i < len; i++) {
      ...
      if (test) {
        ...
        if (~index) results[index].results.push(jest_result_list[i]);
          ...
          results.push({ run_id: test.run_id, results: [jest_result_list[i]] });
      } else {
        results.push({
          case_id: jest_result_list[i].case_id,
          result: jest_result_list[i],
        });
      }
    }
    return results;
